...
var Asteroid = (function()
{
	var create = function()
	{
		...
		obj.radius = 40;
		// obj.color = '#FF5900';
		obj.color = color[Math.floor(Math.random()*10)];
		obj.pos = Vec2D.create(0, 0);
		obj.vel = Vec2D.create(0, 0);
		obj.blacklisted = false;
		obj.type = 'b';
...
var keyDown = false;
var keySpace = false;
var color = ['#FF0000', '#800000', '#FFFF00', '#FF6600', '#00FF00', '#008000', '#00FFFF', '#0000FF', '#FF00FF', '#800080'];
...
function generateAsteroidExplosion(asteroid)
{
	...
	for (i;i > -1;--i)
	{
		...
		p.lifeSpan = 80;
		// p.color = '#FF5900';
		p.color = color[Math.floor(Math.random()*10)];
		p.vel.setLength(20 / p.radius);
		p.vel.setAngle(ship.angle + (1 - Math.random() * 2) * doublePI);
